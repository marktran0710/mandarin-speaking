...
import { SAMPLE_DEBUG_RECORD, type DebugAttemptSource } from "./practice-debug/debugModel";
import { formatBackendError } from "../../utils/storyRecorderFeedback";
import { loadAiProviders, postPracticeAnalysisStream } from "../../shared/api/practice-analysis";
import { canUseDatabase, listCustomStories } from "../../services/database";
import { loadCustomStories, publishedTopicsFromStories } from "@entities/story";
import type { Topic } from "@entities/topic";
import DebugPipelineDetails from "./practice-debug/DebugPipelineDetails";
import { AUDIO_PREPARATION_TIMEOUT_MS, BACKEND_ANALYSIS_TIMEOUT_MS, consumeAnalysisStream, derivePipelineView, sourceLabel, withTimeout, type AnalysisPhase, type DebugProcessingState, type JsonObject, type ProcessingTraceStage, type RecordedRequestContext } from "./practice-debug/debugPipeline";
...
export default function TeacherPracticeDebugPage({ records }: { records: AudioRecord[] }) {
  ...
  useEffect(() => {
    if (!canUseDatabase()) {
      const topics = publishedTopicsFromStories(loadCustomStories());
    ...
    void listCustomStories()
      .then((stories) => {
        if (!active) return;
        // The debugger is also useful while authoring a story locally. If the
        // production content endpoint is unavailable or empty, use the same
        // published local draft source as the teacher story builder; the
        // student verified route still remains server-authoritative.
        const topics = publishedTopicsFromStories(stories.length > 0 ? stories : loadCustomStories());
        setPublishedTopics(topics);
        setSelectedTopicId((current) => current && topics.some((topic) => topic.id === current)
          ? current
          : topics[0]?.id ?? "");
      })
      .catch(() => {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const data = await loadAiProviders(controller.signal);
        if (!data) return;
  ...
  const analyzeRecording = async (rawBlob: Blob, durationOverride?: number) => {
    ...
    try {
      ...
      const response = await postPracticeAnalysisStream(formData, controller.signal);
      if (runId !== analysisRunIdRef.current) return;
      if (!response.ok) {
        let detail = "Practice analysis failed.";
        try {
