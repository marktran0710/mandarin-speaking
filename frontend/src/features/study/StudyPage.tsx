...
export default function StudyPage({ topics, statusByStoryId, onOpenTopic }: StudyPageProps) {
  ...
  const header = (
    <StudentPageHeader
      eyebrowZh="學習"
      eyebrowEn="Study"
      titleZh={currentGroup?.lessonNumber != null ? `第${currentGroup.lessonNumber}課 · ${currentTitle?.zh ?? "課程目錄"}` : "學習中心"}
      titleEn={currentGroup?.lessonNumber != null ? (currentTitle?.en ?? "Course catalogue") : "Course catalogue"}
    />
  );
  ...
  if (groups.length === 0) {
    return (
      <StudentPage
        ...
        header={header}
        state="empty"
        emptyTitle={<><span lang="zh-Hant">尚未指派課程</span> · No lessons have been assigned yet</>}
      />
    );
  }

  const rail = (
    <div className="sa-study__rail">
