...
import AdminContentDoctorPage from "../pages/AdminContentDoctorPage";
...
const NAV_ITEMS = ["Admin Home", "Materials", "Vocabulary", "Content Doctor", "Teachers", "Students", "IRT / Student analytics", "Measurement", "Practice Debug", "ASR Compare", "BKT Debug", "Research"] as const;
...
export default function AdminApp({ embedded = false, onExit, initialNav = "Admin Home" }: { embedded?: boolean; onExit?: () => void; initialNav?: AdminNav } = {}) {
  ...
  const heading = activeNav === "Vocabulary" ? "Vocabulary" : activeNav === "Content Doctor" ? "Content Doctor" : activeNav === "Materials" ? "Materials" : activeNav === "IRT / Student analytics" ? "IRT / Student analytics" : activeNav === "Measurement" ? "Measurement health" : activeNav === "Practice Debug" ? "Practice Stage Debugger" : activeNav === "ASR Compare" ? "ASR Compare" : activeNav === "BKT Debug" ? "BKT Debug" : activeNav === "Research" ? "Research study" : "Account Control Center";
  const description = activeNav === "Content Doctor" ? "Read-only consistency report over curriculum content and media." : activeNav === "Materials" ? "Create and review the story content available to students." : activeNav === "IRT / Student analytics" ? "Track student ability, response quality and calibration readiness." : activeNav === "Measurement" ? "Check how much of the scoring pipeline produced usable evidence." : activeNav === "Practice Debug" ? "Trace student attempts through the scoring pipeline." : activeNav === "ASR Compare" ? "Compare ASR models on the same recording through the real scoring pipeline." : activeNav === "BKT Debug" ? "Inject fake answers and watch BKT mastery replay, step by step." : activeNav === "Research" ? "Study status, assignment balance, and protocol fidelity - never shown on the Teacher Dashboard." : "Manage teacher and student accounts.";
