...
describe("dynamic diagnostic rounds", () => {
  it("creates 15 questions per round with one stable concept and distinct exposure ids", () => {
    ...
    rounds.forEach((questions, index) => {
      ...
      expect(questions.every((question) => question.round === (index + 1) && question.tier === `tier${index + 1}`)).toBe(true);
    });
  ...
  it("keeps rounds 1/3 as four-choice and round 2 as a typed response", () => {
    const round1 = buildDiagnosticRoundQuestions(entries, "tier1");
    const round2 = buildDiagnosticRoundQuestions(entries, "tier2");
    const round3 = buildDiagnosticRoundQuestions(entries, "tier3");
    expect(round1.every((question) => question.options.length === 4)).toBe(true);
    expect(round2.every((question) => question.answerFormat === "free_text" && question.options.length === 0)).toBe(true);
    expect(round3.every((question) => question.answerFormat === "single_choice" && question.options.length === 4)).toBe(true);
    ...
    expect(round3.every((question) => new Set(question.options).size === 4 && question.options.includes(question.correctAnswer))).toBe(true);
  });

  it("prefers the bank's teacher-approved Round 3 MCQ options", () => {
    const entry: VocabQuizEntry = {
      ...
      assessmentQuestions: [
        {
          questionId: "lesson-5-chocolate-R3",
          wordId: "lesson-5-chocolate",
          targetWord: "巧克力",
          pinyin: "qiǎokèlì",
          pos: "N",
        ...
        {
          questionId: "lesson-5-chocolate-R2",
          wordId: "lesson-5-chocolate",
          targetWord: "巧克力",
          pinyin: "qiǎokèlì",
          pos: "N",
  ...
  it("uses the workbook's Round 3 context MCQ", () => {
    const entry: VocabQuizEntry = {
      word: "房間",
      translation: "room",
  ...
  it("uses an approved lesson sentence for Round 3 before an assessment-bank fallback prompt", () => {
    const entry: VocabQuizEntry = {
      ...
      assessmentQuestions: [{
        questionId: "lesson-5-chocolate-R3-sentence",
        wordId: "lesson-5-chocolate",
        targetWord: "巧克力",
        pinyin: "qiǎokèlì",
        pos: "N",
