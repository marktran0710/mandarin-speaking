...
export function QuizQuestion(props: QuizQuestionProps) {
  ...
  // Move focus to the continue button after submission so keyboard users can advance.
  useEffect(() => {
    if (selected) nextButtonRef.current?.focus();
  }, [selected]);
  ...
  return <section className="story-vocab-quiz vocab-quiz-question-screen" aria-label="Vocabulary quiz">
    ...
    <div className="vocab-quiz-content">
      ...
      <div className="vocab-quiz-answer-panel">
        ...
        <div className="vocab-quiz-actions">{selected && <button ref={nextButtonRef} type="button" className="btn-vocab-quiz-next" onClick={next}>{isLast ? <BiLabel zh="看結果" pinyin="Kàn jiéguǒ" en="See results" /> : <BiLabel zh="下一題" pinyin="Xià yì tí" en="Next question" />}</button>}</div>
      </div>
    </div>
  </section>;
}
