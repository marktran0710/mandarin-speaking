...
function QuizScoreTrack({ correct, answered, config, totalQuestions }: { correct: number; answered: number; config: TierConfig; totalQuestions: number }) {
  ...
  return <div className={`vq-track${reachable ? "" : " is-out-of-reach"}`}>
    ...
    <div className="vq-track-notch-row" aria-hidden="true"><span className="vq-track-notch-label" style={{ left: pct(pass) }}>{stars} {pass}</span></div>
    <p className="vq-track-note">{reachable ? need > 0 ? <BiLabel zh={`還要對 ${need} 題`} pinyin={`Hái yào duì ${need} tí`} en={`${need} more correct for ${stars}`} /> : <BiLabel zh={`${stars} 拿到了！`} pinyin={`${stars} Nádào le!`} en={`${stars} earned — keep going`} /> : <BiLabel zh={`這次先練習，下次再拿 ${stars}`} pinyin={`Zhè cì xiān liànxí, xià cì zài ná ${stars}`} en={`Practice run — try for ${stars} next time`} />}</p>
  </div>;
}
...
export function QuizQuestion(props: QuizQuestionProps) {
  ...
  return <section className="story-vocab-quiz vocab-quiz-question-screen" aria-label="Vocabulary quiz">
    <div className="vocab-quiz-topbar">{onBack && <button type="button" className="btn-vocab-quiz-back" onClick={onBack}><BiLabel zh="← 回活動" pinyin="← Huí huódòng" en="← Back to activities" /></button>}{showFinishButton && <button type="button" className="btn-vocab-quiz-finish" onClick={() => finish(results)}><BiLabel zh="結束，看結果" pinyin="Jiéshù, kàn jiéguǒ" en="Finish & see results" /></button>}</div>
