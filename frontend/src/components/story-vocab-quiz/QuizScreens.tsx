...
export function ModeSelectScreen({ stars, weakEntries, priorityReviewWords = [], level = "easy", assessmentQuestionCounts, startTier, chooseWeakWords, showReview }: { stars: 0 | QuizTier; weakEntries: VocabQuizEntry[]; priorityReviewWords?: VocabPriorityReviewWord[]; alreadyCompleted?: boolean; level?: "easy" | "medium" | "hard"; assessmentQuestionCounts?: Partial<Record<VocabAssessmentLevel, number>>; startTier: (mode: TierMode) => void; chooseWeakWords: () => void; showReview: () => void }) {
  const hasFullAssessment = Boolean(assessmentQuestionCounts?.easy && assessmentQuestionCounts.medium && assessmentQuestionCounts.hard);
  ...
  const tierDescription = (card: (typeof TIER_CARDS)[number], config: (typeof TIER_CONFIGS)[TierMode]) => {
    ...
    if (hasFullAssessment && card.mode === "tier1") return { zh: "45 題 — 先簡單，再中等，最後困難。", pinyin: "45 tí — xiān jiǎndān, zài zhōngděng, zuìhòu kùnnán.", en: "45 questions — Easy, then Medium, then Hard." };
...
export function SummaryScreen({ mode, results, missedWords, missedEntries, isRetryRound, isFullAssessment = false, stars, onDone, startTier, practiceMissedWords, backToModes }: { mode: VocabQuizMode | null; results: VocabQuizQuestionResult[]; missedWords: VocabQuizQuestionResult[]; missedEntries: VocabQuizEntry[]; isRetryRound: boolean; isFullAssessment?: boolean; stars: 0 | QuizTier; onDone: () => void; startTier: (mode: TierMode) => void; practiceMissedWords: () => void; backToModes: () => void }) {
  const correctCount = results.filter((result) => result.correct).length;
  const tierConfig = !isRetryRound && !isFullAssessment ? tierConfigFromMode(mode) : null;
