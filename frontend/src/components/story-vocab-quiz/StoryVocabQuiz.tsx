...
export default function StoryVocabQuiz({ entries, onDone, onBack, onComplete, storyId, baseStoryId, level = "easy", studentId, studentName }: {
  ...
  // Epic 4, Task 4.9/4.10: an active research participant's practice dosage
  // is a fixed, server-controlled budget. Manual one-word practice and
  // Lesson Challenge are both extra, self-selected graded practice outside
  // that budget, so both are switched off for the duration of the study
  // rather than left reachable alongside the server-selected session.
  const researchActive = getCachedResearchContext().active;
  if (session.screen === "mode-select") return <ModeSelectScreen stars={session.stars} weakEntries={session.weakEntries} interimReviewEntries={session.interimReviewEntries} priorityReviewWords={session.priorityReviewWords} strongWords={session.strongWords} dueWords={session.dueWords} chooseDueReview={researchActive ? session.startResearchReview : () => { if (dueEntries.length > 0) { session.setIsRetryRound(false); session.chooseMode("maintenance_review", dueEntries, dueEntries.length); } }} researchDueCount={session.researchDueEntries.length} assessmentQuestionCounts={assessmentQuestionCounts} progress={session.lessonProgress} startTier={session.startTier} chooseWeakWords={researchActive ? session.startResearchPractice : () => { session.setIsRetryRound(false); session.chooseMode("weak_words", session.weakEntries, session.weakEntries.length); }} chooseInterimReview={() => { session.setIsRetryRound(false); session.chooseMode("weak_words", session.interimReviewEntries, session.interimReviewEntries.length); }} onPracticeWord={researchActive ? undefined : (word) => { const entry = entries.find((e) => (word.wordId && e.wordId === word.wordId) || e.word === word.word); if (entry) session.practiceWord(entry); }} onContinue={() => { if (!session.lessonProgress.knowIt.completed) session.startTier("tier1"); else if (!session.lessonProgress.sayIt.completed) session.startTier("tier2"); else if (!session.lessonProgress.useIt.completed) session.startTier("tier3"); else session.setIsRetryRound(false); }} onFinish={onDone} startChallenge={researchActive ? undefined : session.showChallengeEntry} showReview={() => session.setScreen("review")} researchMode={researchActive} />;
