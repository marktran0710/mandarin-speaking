...
import type { PraatMetrics, Topic, WordProsody } from "../story-recorder/StoryRecorder";
  ...
  return (
    <section className="sfc-progress-snapshot" aria-label="Practice progress">
      ...
      <div className="sfc-progress-snapshot-grid">
        <div className="snap-cell snap-cell-1"><strong>{attempts}</strong><span>次數 / Attempts</span></div>
        <div className="snap-cell snap-cell-2"><strong>{typeof passed === "number" && typeof total === "number" ? `${passed}/${total}` : "—"}</strong><span>音節 / Syllables</span></div>
        <div className="snap-cell snap-cell-3"><strong>{practicePartCount}</strong><span>待練 / To practise</span></div>
      </div>
    </section>
  );
}
