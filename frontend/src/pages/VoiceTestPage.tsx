...
export default function VoiceTestPage() {
  ...
  const primaryLabel = isRecording ? { zh: "停止，看回饋", pinyin: "Tíngzhǐ, kàn huíkuì", en: "Stop and get feedback" } : metrics ? { zh: "再錄一次", pinyin: "Zài lù yí cì", en: "Record again" } : { zh: "開始語音測試", pinyin: "Kāishǐ yǔyīn cèshì", en: "Start voice test" };
   return <StudentPageShell template="activity" pageId="voice-practice"><StudentPageBody variant="stage" className="voice-test-page"><StudentPageHeader eyebrow={{ zh: "語音練習", en: "Voice practice" }} title={{ zh: "分析你的聲音", en: "Analyze Your Voice" }} lede={{ zh: "錄音或上傳 WAV 檔案，系統會轉錄音檔，然後檢查發音和語言表現，給你回饋。", en: "Record or upload a WAV file. The system transcribes the audio, then checks pronunciation and language feedback from the recording." }} />
    <section className="voice-test-hero"><div className="voice-test-status"><span><BiLabel zh="狀態" en="Status" /></span><strong>{isRecording ? <BiLabel zh="錄音中" en="Recording" /> : isAnalyzing ? <BiLabel zh="分析中" en="Analyzing" /> : <BiLabel zh="準備好了" en="Ready" />}</strong><p>{isRecording ? <BiLabel zh={`已錄音 ${recordingDuration} 秒`} en={`${recordingDuration}s recorded`} /> : <BiLabel zh="錄一次就夠了。" en="One recording is enough." />}</p></div></section>
    <section className="voice-test-workspace"><StudentAudioActionPanel className="voice-test-controls" primaryIcon={metrics ? "retry" : "record"} primaryLabel={primaryLabel.en} uploadLabel="Import WAV file" accept=".wav,audio/wav,audio/wave,audio/x-wav,audio/vnd.wave" onPrimaryAction={isRecording ? stopRecording : startRecording} onFileChange={handleImportWav} isRecording={isRecording} isAnalyzing={isAnalyzing} hasPendingAudio={Boolean(pendingAudio && !metrics)} onAnalyze={pendingAudio && !metrics ? () => void analyzeAudio() : undefined} status={isRecording ? `${recordingDuration}s recorded` : isAnalyzing ? "Running voice analysis..." : "One recording is enough."} readyMessage={null} />
      <div className="voice-step-row voice-step-row-legacy" aria-label="Voice test steps"><span><BiLabel zh="1. 說話或上傳" en="1. Speak or upload" /></span><span><BiLabel zh="2. 轉錄音檔" en="2. Transcribe audio" /></span><span><BiLabel zh="3. 查看結果" en="3. Review" /></span></div>
