...
Holds the analytics computation that used to live inline in
routers/knowledge_analytics.py. Each public compute function opens its own
``connect_db()`` block (rather than taking an already-open connection),
matching the original router functions exactly - the router calls these
through a TTL-cached, threadpool-dispatched wrapper rather than a single
per-request connection block, so the connection lifecycle has always lived
here, one level below the endpoint.
"""
...
def _load_records(
    ...
    with connect_db() as db:
        ...
        rows = repo.find_student_names(db, ids)
...
def compute_bkt_question_audit(all_tiers: bool = False) -> dict[str, Any]:
    """Read-only admin audit of the material and response-quality evidence."""
    with connect_db() as db:
        stories = [dict(row) for row in repo.list_published_stories_for_audit(db)]
        attempts = [dict(row) for row in repo.list_quiz_attempts_for_audit(db)]
