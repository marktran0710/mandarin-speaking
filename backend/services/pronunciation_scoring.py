"""Sentence-level pronunciation grading: gates every syllable diagnosis on
recording quality, decides whether the recording passes as a whole, and
builds the plain-language vowel/tone-direction descriptions shown to the
student.

The actual per-syllable tone measurement lives in chinese_tones.py/
praat_analyzer.py/tone_decision.py — dedicated, already-separate modules
this file calls into but does not restructure. This module is the layer
above them: turning per-syllable diagnostics into one sentence-level
pass/fail verdict.
"""
