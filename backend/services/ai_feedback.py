...
Two entry points:
  - generate_language_feedback() / generate_story_feedback(): text feedback
    for a per-scene attempt or a whole-story narration. Both try the
    requested cloud provider (groq/gemini/openai) first, degrade through the
    other configured providers on failure, and finally fall back to the
...
an acoustic score is trustworthy enough to show as a real score rather than
a "could not be scored safely" placeholder.
"""
