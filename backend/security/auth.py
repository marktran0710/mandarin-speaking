...
load_dotenv()
# Preserve the pre-migration repository-level lookup and also support a
# backend-local override for the new package location.  ``load_dotenv`` does
# not override an already loaded value, so the historical path keeps the same
# precedence it had before this module moved under ``security/``.
